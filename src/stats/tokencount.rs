...
pub struct CounterToken {
    ...
    key_cache: FxDashMap<(String, String), Arc<str>>, // Cache for key reuse
}
...
impl CounterToken {
    ...
    /// Number of tokens/sessions that are still valid (not expired).
    ...
    /// Detail of active (non-expired) sessions, grouped by user.
    /// Same logic as `get_all_tokens_json`, but filtered to valid tokens only.
    pub fn get_active_sessions_json(&self) -> Vec<AllTokenUsage> {
        let now = Utc::now();
        let mut grouped: HashMap<String, Vec<TokenUsage>, BuildHasherDefault<FxHasher>> =
        HashMap::with_hasher(BuildHasherDefault::<FxHasher>::default());
        grouped.reserve(self.counts.len() / 2);
        ...
        for entry in self.counts.iter() {
            ...
            if usage.expire_at <= now {
                continue; // expired token, skip it
            }
            let key = entry.key();
            if let Some((user, token_id)) = key.split_once(':') {
                let count = usage.count.load(Ordering::Relaxed);
                grouped
                .entry(user.to_string())
                .or_insert_with(|| Vec::with_capacity(4))
                .push(TokenUsage {
    ...
    /// Purge expired entries so the map doesn't grow indefinitely.
    /// Cleans up both `counts` and `key_cache` to keep memory usage bounded.
    /// Returns the number of expired token entries removed.
    pub fn purge_expired(&self) -> usize {
        let now = Utc::now();

        let before = self.counts.len();
        self.counts.retain(|_, usage| usage.expire_at > now);
        let removed = before - self.counts.len();

        self.key_cache
        .retain(|_, cached_key| self.counts.contains_key(cached_key));
